...
# Toggle via an environment variable (e.g., export USE_NVTX=1) 
# or a simple boolean flag
PROFILING_MODE = os.environ.get("USE_NVTX", "0") == "1"
...
def nvtx_range(name: str):
    if PROFILING_MODE:
        ...
        # No-op context manager when profiling is turned off
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        with nvtx_range("transformer attention sublayer"):
            x_attn = self.attn(self.ln1(x))
        attn_sublayer_output = x + x_attn

        # Apply the feed-forward sublayer
        with nvtx_range("transformer swiglu ffn sublayer"):
...
def scaled_dot_product_attention(
    ...
    d_k = K.shape[-1]
    with nvtx_range("attention sublayer: computing attention scores"):
        attention_scores = einsum(Q, K, "... query d_k, ... key d_k -> ... query key") / math.sqrt(d_k)

    if mask is not None:
        attention_scores = torch.where(mask, attention_scores, float("-inf"))

    with nvtx_range("attention sublayer: computing softmax scores"):
        attention_weights = softmax(attention_scores, dim=-1)  # Softmax over the key dimension

    with nvtx_range("attention sublayer: computing final matmul"):
